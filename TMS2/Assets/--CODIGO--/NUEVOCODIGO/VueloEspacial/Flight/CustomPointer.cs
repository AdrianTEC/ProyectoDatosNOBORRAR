...
public class CustomPointer : MonoBehaviour {
	...
	void OnGUI() {
		...
		if (pointerTexture != null && !center_lock)
			GUI.DrawTexture(new Rect(pointerPosition.x - (size / 2), Screen.height - pointerPosition.y - (size / 2),size, size), pointerTexture);
		else {
		
			GUI.DrawTexture(new Rect((Screen.width / 2f) - (size / 2), (Screen.height / 2f) - (size / 2), size, size), pointerTexture);
