...
namespace _Scripts.Player.VueloEspacial.Flight
{
	[System.Serializable]
	public class CameraFlightFollow : MonoBehaviour {
		...
		void FixedUpdate () {
			...
			Vector3 positionDifference = target.position - position;
			//Move the camera towards the new position.
			position = Vector3.Lerp (position, newPosition, Time.deltaTime * follow_tightness);
